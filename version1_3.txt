...
try:
    ...
    # and logging the error for further investigation.
...
# Define a function to read content in a female voice
def read_content(content, volume=4.5):
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            message = "Please enter symptoms separated by commas."
            ...
            try:
                ...
                # Read the topic and content in a female voice
                read_content("Disease Description")
                read_content(dis_des)
                read_content("Precautions")
                for precaution in precautions[0]:
                    read_content(precaution)
                read_content("Medications")
                for medication in medications:
                    read_content(medication)
                read_content("Diets")
                for diet in rec_diet:
                    read_content(diet)
                read_content("Workouts")
                for wrkout in workout:
                    read_content(wrkout)
